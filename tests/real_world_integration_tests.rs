...
async fn test_real_postman_collection_blazemeter() -> Result<()> {
    ...
    assert!(newman_post.exists(), "Newman POST request should be imported");
    
    let newman_json = output_dir.join("newman__post_request_with_json_body.yaml");
    assert!(newman_json.exists(), "Newman JSON POST request should be imported");
...
async fn test_real_usps_postman_collection() -> Result<()> {
    ...
    assert!(addresses_folder.exists(), "Addresses folder should be created");
    
    let tracking_folder = output_dir.join("tracking");
    assert!(tracking_folder.exists(), "Tracking folder should be created");
...
async fn test_real_openapi_redocly_template() -> Result<()> {
    ...
    let spec_url = "https://raw.githubusercontent.com/Redocly/openapi-template/gh-pages/openapi.yaml";
...
async fn test_real_swagger_petstore_openapi() -> Result<()> {
    ...
    assert!(get_inventory.exists(), "Get inventory test should be generated");
