...
async fn test_postman_collection_import() -> Result<()> {
    ...
    assert!(nested_test_file.exists(), "Nested test file should be created");
...
async fn test_unsupported_import_tool() {
    ...
    assert!(result.unwrap_err().to_string().contains("Unsupported import tool"));
}
...
async fn test_invalid_postman_collection() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Failed to parse Postman collection"));
}