...
async fn test_github_massive_openapi_spec() -> Result<()> {
    ...
    assert!(
        test_files.len() > 1000,
        "Should generate 1000+ tests from GitHub API, got {}",
        test_files.len()
    );
    ...
    assert!(
        repos_create.exists(),
        "GitHub Repos API should be generated"
    );
...
async fn test_auth0_complex_nested_postman_collection() -> Result<()> {
    ...
    assert!(
        db_connections.exists(),
        "DB Connections folder should exist"
    );

    let get_access_token = output_dir.join("get_access_token");
    assert!(
        get_access_token.exists(),
        "Get Access Token folder should exist"
    );
    ...
    assert!(
        auth_code_test.exists(),
        "Authorization code flow test should exist"
    );

    let client_creds_test = get_access_token.join("get_access_token_client_credentials.yaml");
    assert!(
        client_creds_test.exists(),
        "Client credentials flow test should exist"
    );

    let refresh_token_test = get_access_token.join("get_access_token_refresh_token.yaml");
    assert!(
        refresh_token_test.exists(),
        "Refresh token flow test should exist"
    );
...
async fn test_openai_large_integer_overflow_handling() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "OpenAI spec should fail due to integer overflow"
    );
    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("invalid type: integer")
            || error_msg.contains("Failed to parse OpenAPI"),
        "Should get integer parsing error, got: {}",
        error_msg
    );
...
async fn test_usps_large_production_collection() -> Result<()> {
    ...
    assert!(
        test_files.len() >= 40,
        "Should generate 40+ tests from USPS API, got {}",
        test_files.len()
    );
...
async fn test_edge_case_postman_formats() -> Result<()> {
    ...
    assert!(
        null_values_test.exists(),
        "Null values test should be created"
    );
    ...
    assert!(
        complex_url_test.exists(),
        "Complex URL test should be created"
    );

    let url_content = fs::read_to_string(&complex_url_test)?;
    // Should contain the raw URL from the collection
    assert!(
        url_content.contains("https://{{host}}/{{version}}/test?param1={{value1}}&param2=static")
    );

    let empty_body_test = output_dir.join("empty_body.yaml");
    assert!(
        empty_body_test.exists(),
        "Empty body test should be created"
    );
...
async fn test_performance_with_large_specs() -> Result<()> {
    ...
    let mut large_spec = String::from(
        r#"
openapi: 3.0.0
info:
...
"#,
    ...
    assert!(
        duration.as_secs() < 30,
        "Large spec generation took too long: {:?}",
        duration
    );
...
async fn test_malformed_collections_error_handling() -> Result<()> {
    ...
    let test_cases = vec![
        ...
        (
            "invalid_items.json",
            r#"{ "info": { "name": "Test", "schema": "v2.1.0" }, "item": "not_an_array" }"#,
        ),
        ("empty_file.json", ""),
    ];
    ...
    for (filename, content) in test_cases {
        ...
        assert!(
            result.is_err(),
            "Malformed collection {} should fail",
            filename
        );
    }
...
async fn test_unicode_and_special_characters() -> Result<()> {
    ...
    assert!(
        !test_files.is_empty(),
        "Should create test files from unicode collection"
    );
